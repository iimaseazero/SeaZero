...
export default function Home() {
  ...
  return (
    <main className="min-h-screen xl:h-screen flex flex-col overflow-y-auto xl:overflow-hidden relative">
      {/* ─── Mobile View Switcher Tabs (< lg screens) ─── */}
      <div className="xl:hidden flex border-b border-[var(--border)] bg-[var(--background)] px-4 py-2 gap-2 z-10 sticky top-0">
        {[
      ...
      {/* ─── Layout container ─── */}
      <div className="flex-1 flex flex-col xl:flex-row min-h-0 overflow-y-auto xl:overflow-hidden relative z-10">
        {/* LEFT — Config Panel */}
        <motion.aside
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ delay: 0.1 }}
          className={`w-full xl:w-[280px] flex-shrink-0 flex-col xl:h-full border-b xl:border-b-0 xl:border-r border-[var(--border)] bg-[var(--background)] ${
            mobileTab === 'config' ? 'flex h-[600px] xl:h-full' : 'hidden xl:flex'
          }`}
        >
          <ConfigPanel />
        </motion.aside>
        ...
        <motion.section
          ...
          className={`flex-1 overflow-y-auto px-4 sm:px-6 py-4 sm:py-5 space-y-4 sm:space-y-5 xl:h-full ${
            mobileTab === 'analytics' ? 'block' : 'hidden xl:block'
          }`}
          style={{ minWidth: 0 }}
        >
          <VerdictBanner />
        ...
        <motion.aside
          ...
          className={`w-full xl:w-[440px] flex-shrink-0 p-3 sm:p-4 flex flex-col bg-[var(--background)] border-t xl:border-t-0 xl:border-l border-[var(--border)] ${
