...
export default function VerdictBanner() {
  ...
  return (
    <motion.div
      ...
      key={`${feasible}-${isIce}`}
      className="w-full flex items-start gap-3 px-5 py-3 rounded-xl"
      style={{
    ...
    >
      ...
      />
      <span
        className="text-sm font-semibold tracking-wide uppercase flex-shrink-0 mt-0.5"
        style={{ fontFamily: 'var(--font-display)', color: accent }}
      >
        {label}
      </span>
      <span className="w-px self-stretch flex-shrink-0" style={{ background: feasible ? 'rgba(56,217,200,0.2)' : 'rgba(239,68,68,0.2)' }} />
      <span
        className="text-xs flex-1 leading-relaxed mt-0.5"
